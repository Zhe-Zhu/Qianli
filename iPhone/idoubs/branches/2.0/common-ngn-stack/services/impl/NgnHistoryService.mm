...
-(BOOL) databaseLoad{
	BOOL ok = YES;
	static const char *sqlStatement = "SELECT id,seen,status,mediaType,remoteParty,start,end,content FROM hist_event ORDER BY start DESC LIMIT 15";
    ...
    ret = sqlite3_prepare_v2([storageService database], sqlStatement, -1, &compiledStatement, NULL);
	if(ret == SQLITE_OK) {
		while(sqlite3_step(compiledStatement) == SQLITE_ROW) {
			...
			switch (mediaType) {
				...
				case MediaType_SMS:
					...
					break;
                case MediaType_Image: //LG
                    event = [(NgnHistoryEvent*)[NgnHistoryEvent createImageEventWithStatus: status andRemoteParty: remoteParty andContent: [NSData dataWithBytes: content length: contentLength]] retain];
                    break;
				case MediaType_Chat:
...
-(BOOL) databaseLoadWithRemoteParty:(NSString *)remoteParty WithEntriesLength:(int) length{
	BOOL ok = YES;
    
    NSString *str = [NSString stringWithFormat:@"SELECT id,seen,status,mediaType,remoteParty,start,end,content FROM hist_event WHERE remoteParty = '%@' ORDER BY start DESC LIMIT %d", remoteParty, length];
    const char *sqlStatement = [str cStringUsingEncoding:NSUTF8StringEncoding];
	sqlite3_stmt *compiledStatement = nil;
	NgnHistoryEvent* event = nil;
	int ret;
...
-(BOOL) databaseAddEvent: (NgnHistoryEvent*)event{
	...
	if(sqlStatement){
		if(ret == SQLITE_OK) {
            ...
            else if ([event isKindOfClass: [NgnHistoryImageEvent class]]){
                // Added by LG
                sqlite3_bind_blob(compiledStatement, 7, [((NgnHistoryImageEvent*)event).content bytes], [((NgnHistoryImageEvent*)event).content length], SQLITE_STATIC);
            }
			else {
...
-(BOOL) loadWithRemoteParty:(NSString *)remoteParty WithEntriesLength:(int)lenght{
	BOOL ret;
	@synchronized(self){
		ret = [self databaseLoadWithRemoteParty:remoteParty WithEntriesLength:lenght];
	}
	return ret;
}
